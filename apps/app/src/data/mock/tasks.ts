...
export const generateTaskDates = () => {
  ...
  dueDate.setDate(now.getDate() + 14 + Math.floor(Math.random() * 60))
...
export type Task = (typeof baseTasks)[number] & {
  createdDate: string
  dueDate: string
  estimatedTime: (typeof estimatedTimes)[number]
  sprintCycle: (typeof sprintCycles)[number]
}
