...
export const columns: ColumnDef<Webhook>[] = [
  {
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-2">
        <CopyButton className="size-6 scale-100 rounded-full" text="Copt Api Key" />
        <LongText className="w-28">{row.getValue("id")}</LongText>
        <div className="w-3 max-w-36" />
      </div>
    ),
  ...
  {
    accessorKey: "url",
    header: ({ column }) => <DataTableColumnHeader column={column} title="URL Endpoint" />,
    cell: ({ row }) => (
      <Link
