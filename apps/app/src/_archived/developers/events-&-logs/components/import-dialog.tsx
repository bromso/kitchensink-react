...
export default function ImportDialog() {
  ...
  const uploadFile = useCallback((file: FileWithPreview) => {
    let progress = 0
    const intervalId = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        setTimeout(() => {
          setFiles((prevFiles) =>
            prevFiles.map((f) => (f.name === file.name ? { ...f, status: "success" } : f))
          )
        }, 500)
      }
    }, 300)
    ...
    if (Math.random() < 0.1) {
      setTimeout(() => {
        clearInterval(intervalId)
        setFiles((prevFiles) =>
          prevFiles.map((f) => (f === file ? { ...f, status: "error", progress: 100 } : f))
        )
      }, 2000)
    }
  }, [])
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        >
          ...
          <label
            ...
            htmlFor="file-upload"
          >
            <Icon icon="lucide:upload" strokeWidth={1.4} width={22} height={22} />
                  ...
                  <div className="flex flex-1 flex-col items-start gap-2">
                    <div className="flex w-full items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Icon icon="lucide:file" className="opacity-50" width={22} height={22} />
                    ...
                    <div className="flex w-full items-center justify-between gap-3">
                      ...
                      <div className="flex w-[30px] items-center justify-end">
                        ...
                        {file.status === "error" && (
                          <Icon icon="lucide:alert-circle" className="h-4 w-4 text-red-500" />
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <PopoverContent className="w-80">
              ...
              <div className="bg-muted mt-4 rounded-md p-3">
                <div className="mb-2 flex items-center">
                  <Icon icon="lucide:alert-circle" className="mr-2 h-4 w-4" />
