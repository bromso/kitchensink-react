...
const columns: { id: ArtefactStatus; label: string; icon: string; color: string }[] = [
  { id: "draft", label: "Draft", icon: "tabler:pencil", color: "bg-zinc-500/20 text-zinc-600" },
  { id: "review", label: "In Review", icon: "tabler:eye", color: "bg-amber-500/20 text-amber-600" },
  {
    id: "approved",
    label: "Approved",
...
export function KanbanBoard({ campaignId, className }: KanbanBoardProps) {
  // Local state to track artefact positions (for drag and drop)
  ...
  const { artefacts: fetchedArtefacts, loading } = campaignId
    ? campaignArtefactsResult
    : allArtefactsResult

  // Sync local state with fetched artefacts when data loads
  useEffect(() => {
    if (!loading && fetchedArtefacts.length > 0) {
      setArtefacts(fetchedArtefacts)
    } else if (!loading && fetchedArtefacts.length === 0 && artefacts.length > 0) {
      // Clear local state when switching to a campaign with no artefacts
      setArtefacts([])
    }
  }, [fetchedArtefacts, loading])
  ...
  // Group artefacts by status
  const artefactsByStatus = columns.reduce(
    (acc, column) => {
      acc[column.id] = artefacts.filter((a) => a.status === column.id)
      return acc
    },
    {} as Record<ArtefactStatus, Artefact[]>
  )

  const activeArtefact = activeId ? artefacts.find((a) => a.id === activeId) : null
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    const activeArtefactId = active.id as string
    const overId = over.id as string

    // Check if we're dropping over a column or another artefact
    const overColumn = columns.find((c) => c.id === overId)
    const overArtefact = artefacts.find((a) => a.id === overId)

    const activeArtefact = artefacts.find((a) => a.id === activeArtefactId)
    if (!activeArtefact) return

    let targetStatus: ArtefactStatus | null = null

    if (overColumn) {
      targetStatus = overColumn.id
    } else if (overArtefact) {
      targetStatus = overArtefact.status
    }

    if (targetStatus && activeArtefact.status !== targetStatus) {
      setArtefacts((prev) =>
        prev.map((a) => (a.id === activeArtefactId ? { ...a, status: targetStatus } : a))
      )
    }
  }
  ...
  if (loading) {
    return (
      <div className={cn("flex min-h-0 flex-1 gap-4 overflow-x-auto p-6 pb-4", className)}>
        {columns.map((column) => (
          <div
            key={column.id}
            className="bg-muted/30 flex h-full max-h-full min-w-[300px] flex-col rounded-xl border"
          >
            ...
            <div className="flex flex-col gap-2 p-3">
              {Array.from({ length: 2 }).map((_, i) => (
                <div key={i} className="bg-card rounded-lg border p-3 shadow-sm">
  ...
  return (
    <>
      ...
      >
        <div className={cn("flex min-h-0 flex-1 gap-4 overflow-x-auto p-6 pb-4", className)}>
          {columns.map((column) => (
            <KanbanColumn
              key={column.id}
              column={column}
              artefacts={artefactsByStatus[column.id]}
        ...
        <DragOverlay>
          {activeArtefact && <KanbanCard artefact={activeArtefact} isDragging />}
        </DragOverlay>
      </DndContext>
...
function KanbanColumn({ column, artefacts, onArtefactClick }: KanbanColumnProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        "bg-muted/30 flex h-full max-h-full min-w-[300px] flex-col rounded-xl border transition-colors",
        isOver && "border-primary/50 bg-primary/5"
      )}
    >
      {/* Column Header */}
      <div className="flex items-center gap-2 border-b p-4">
        <div className={cn("flex h-7 w-7 items-center justify-center rounded-md", column.color)}>
          <Icon icon={column.icon} className="h-4 w-4" />
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        <SortableContext items={artefacts.map((a) => a.id)} strategy={verticalListSortingStrategy}>
          <div className="flex flex-col gap-2 p-3 pb-24">
            {artefacts.length === 0 ? (
              <div className="text-muted-foreground flex h-24 items-center justify-center rounded-lg border border-dashed text-center text-xs">
                Drop artefacts here
              </div>
            ) : (
...
function SortableKanbanCard({ artefact, onClick }: SortableKanbanCardProps) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
    id: artefact.id,
    data: { type: "artefact", artefact },
  })
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <KanbanCard artefact={artefact} isDragging={isDragging} onClick={onClick} />
    </div>
  )
}
...
function KanbanCard({ artefact, isDragging, onClick }: KanbanCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={cn(
        "bg-card hover:bg-accent w-full cursor-grab rounded-lg border p-3 text-left shadow-sm transition-all active:cursor-grabbing",
    ...
    >
      ...
      <div className="mb-2 flex items-start gap-2">
        <div className="bg-muted flex h-8 w-8 shrink-0 items-center justify-center rounded-md">
          <Icon icon={getArtefactIcon(artefact.type)} className="text-muted-foreground h-4 w-4" />
        </div>
        <div className="min-w-0 flex-1">
          <h4 className="truncate text-sm font-medium">{artefact.name}</h4>
      ...
      {artefact.description && (
        <p className="text-muted-foreground mb-2 line-clamp-2 text-xs">{artefact.description}</p>
      )}

      {/* Footer */}
      <div className="text-muted-foreground flex items-center gap-2 text-[10px]">
        {artefact.format && <span className="font-mono">{artefact.format}</span>}
        {artefact.format && artefact.dimensions && <span>·</span>}
        {artefact.dimensions && (
          <span>
